...
abstract class _Controller extends \IPS\Dispatcher\Controller
{
    ...
    protected function manage()
    {
        ...
        /* Columns we need */
        $table->include = array( static::$_prefix."_name", static::$_prefix."_author", static::$_prefix."_directory", 'version', 'long_version', 'updated_at' );
        ...
        /* Specify the buttons */
        $rootButtons = array();
        if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.static::$_prefix.'_import' ) )
        {
            $rootButtons['import'] = array(
                'icon'		=> 'upload',
                'title'		=> 'versions_import',
                'link'		=> \IPS\Http\Url::internal( static::$_queryString."&do=import" ),
                'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack( 'versions_'.static::$_prefix.'_import_title' ) )
            );
        }
        if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.static::$_prefix.'_track' ) )
        {
            $rootButtons['track'] = array(
                'icon'		=> 'files-o',
                'title'		=> 'versions_track',
                'link'		=> \IPS\Http\Url::internal( static::$_queryString."&do=track" ),
                'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack( 'versions_'.static::$_prefix.'_track_title' ) )
            );
        }
        if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.static::$_prefix.'_create' ) )
        {
            $rootButtons['create'] = array(
                'icon'		=> 'plus',
                'title'		=> 'versions_create',
                'link'		=> \IPS\Http\Url::internal( static::$_queryString."&do=create" ),
                'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack( 'versions_create' ) )
            );
        }
        ...
        $table->rowButtons = function( $row ) use ( $versionClass, $self )
        {
            ...
            if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.$self::$_prefix.'_bump' ) )
            {
                $return['bump'] = array(
                    'icon'		=> 'chevron-up',
                    'title'		=> 'versions_bump',
                    'link'		=> \IPS\Http\Url::internal( $self::$_queryString."&do=versionBump&id=" ) . $version->id,
                    'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack( 'versions_bump' ) )
                );
            }

            if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.$self::$_prefix.'_edit' ) )
            {
                $return['edit'] = array(
                    'icon'		=> 'pencil',
                    'title'		=> 'versions_edit',
                    'link'		=> \IPS\Http\Url::internal( $self::$_queryString."&do=edit&id=" ) . $version->id,
                    'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack( 'versions_edit' ) )
                );
                $returnInstalled[1] = $return['edit'];
            }

            if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'versions', 'manage', 'versions_'.$self::$_prefix.'_delete' ) )
            {
                $return['delete'] = array(
                    'icon'		=> 'times-circle',
            ...
            $return['update_url'] = array(
                ...
                'link'		=> \IPS\Http\Url::internal( "app=versions&module=versions&controller=".$self::$_publicController."&directory={$version->{$self::$_prefix.'_directory'}}", 'front', $self::$_seoTemplate ),
                'target'	=> '_blank'
            );
        ...
        /* Display */
        \IPS\Output::i()->title		= \IPS\Member::loggedIn()->language()->addToStack( 'versions_'.static::$_prefix.'_versions' );
        \IPS\Output::i()->output	= \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'members', (string) $table );
    }
    ...
    public function create()
    {
        ...
        if ( $values = $form->values() )
        {
            ...
            $versionData[static::$_prefix.'_author']    = !empty( $values["versions_".static::$_prefix."_author"] ) ? $values["versions_".static::$_prefix."_author"] : NULL;
            $versionData['version']                     = $values['versions_version'];
            $versionData['long_version']                = $values['versions_long_version'];
            $versionData['update_url']                  = !empty( $values['versions_update_url'] ) ? $values['versions_update_url'] : NULL;
            $versionData['updated_at']                  = \IPS\DateTime::create();
            $version = $versionClass::createOrUpdate( $versionData );

            \IPS\Output::i()->redirect(
                \IPS\Http\Url::internal( static::$_queryString ),
                \IPS\Member::loggedIn()->language()->addToStack( 'versions_create_successful', TRUE,
                    array( 'sprintf' => $version->{static::$_prefix.'_name'} ) ), 302 );
        }
        ...
        \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'versions_create', $form, FALSE );
    }
    ...
    public function import()
    {
        ...
        $form = new \IPS\Helpers\Form();
        $form->add( new \IPS\Helpers\Form\Upload( 'versions_'.static::$_prefix.'_import', NULL, TRUE,
        ...
        if ( $values = $form->values() )
        {
            ...
            {
                \IPS\Output::i()->error( "versions_error_".static::$_prefix."Import", "VERSIONS_".strtoupper( static::$_prefix )."_NOT_VALID" );
            }

            \IPS\Output::i()->redirect(
                \IPS\Http\Url::internal( static::$_queryString ),
                \IPS\Member::loggedIn()->language()->addToStack( "versions_import_successful", TRUE,
                    array( 'sprintf' => $version->{static::$_prefix.'_name'} ) ), 302
            );
        }
        ...
        \IPS\Output::i()->title	= \IPS\Member::loggedIn()->language()->addToStack( 'versions_'.static::$_prefix.'_import_title' );
        \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'versions_import', $form, FALSE );
    }
    ...
    public function versionBump()
    {
        ...
        try {
            ...
            return \IPS\Output::i()->error( 'node_error', "VERSIONS_".strtoupper( static::$_prefix )."_NOT_FOUND", 404 );
        }
        ...
        if ( $values = $form->values() )
        {
            ...
            \IPS\Output::i()->redirect(
                \IPS\Http\Url::internal( static::$_queryString ),
                \IPS\Member::loggedIn()->language()->addToStack( 'versions_version_bump_successful', TRUE,
                    array( 'sprintf' => array( $version->{static::$_prefix.'_name'}, $version->version ) ) ), 302 );
        }
        ...
        \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'versions_bump', $form, FALSE );
    }
    ...
    public function edit()
    {
        ...
        {
            $form->add( new \IPS\Helpers\Form\Text( "versions_".static::$_prefix."_name", $version->{static::$_prefix.'_name'}, TRUE ) );
            $form->add( new \IPS\Helpers\Form\Text( "versions_".static::$_prefix."_directory", $version->{static::$_prefix.'_directory'}, TRUE ) );
            $form->add( new \IPS\Helpers\Form\Text( "versions_".static::$_prefix."_author", $version->{static::$_prefix.'_author'} ) );
            $form->add( new \IPS\Helpers\Form\Text( 'versions_version', $version->version, TRUE ) );
            $form->add( new \IPS\Helpers\Form\Number( 'versions_long_version', $version->long_version, TRUE ) );
        }
        ...
        if ( $values = $form->values() )
        {
            ...
            if ( !$version->installed )
            {
                ...
                $version->{static::$_prefix.'_author'}    = !empty( $values["versions_".static::$_prefix."_author"] ) ? $values["versions_".static::$_prefix."_author"] : NULL;
                $version->version       = $values['versions_version'];
                $version->long_version  = $values['versions_long_version'];
            }
            ...
            \IPS\Output::i()->redirect(
                \IPS\Http\Url::internal( static::$_queryString ),
                \IPS\Member::loggedIn()->language()->addToStack( 'versions_edit_successful', TRUE,
                    array( 'sprintf' => $version->{static::$_prefix.'_name'} ) ), 302 );
        }
        ...
        \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'versions_edit', $form, FALSE );
    }
