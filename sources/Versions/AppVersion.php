...
class _AppVersion extends \IPS\versions\Versions
{
	...
	public static function getImportData( array $formValues )
	{
		...
		try
		{
			...
			$appData = json_decode(
				file_get_contents( "phar://" . $formValues[ 'versions_app_import' ] . '/data/application.json' ), TRUE
			);
			$versionData = json_decode(
				file_get_contents( "phar://" . $formValues[ 'versions_app_import' ] . '/data/versions.json' ), TRUE
			);
		...
		{
			throw new ApplicationNotValidException(
				\IPS\Member::loggedIn()->language()->get( 'application_notvalid' )
			);
		}
