...
/**
 * Manage applications
 * @package IPS\versions\modules\admin\manage
 */
class _applications extends \IPS\versions\Versions\Controller
{
    ...
    public function track()
    {
        ...
        foreach ($applications as $key => $application)
        {
            ...
            /* Filter out IPS/system applications, since we have no reason to ever track those */
            if ( in_array( $application->directory, array( 'core', 'chat', 'forums', 'calendar', 'cms', 'downloads', 'blog', 'gallery', 'nexus' ) ) ) {
                unset( $applications[ $key ] );
            } else {
                $selectApps[ $key ] = \IPS\Member::loggedIn()->language()->addToStack( "__app_{$application->directory}" );
            }
        }
        ...
        $form->add( new \IPS\Helpers\Form\Select( 'versions_app_select', NULL, TRUE, array( 'options' => $selectApps ) ) );
        ...
        if ( $values = $form->values() )
        {
            ...
            \IPS\Output::i()->redirect(
                ...
                \IPS\Member::loggedIn()->language()->addToStack( 'versions_track_successful', TRUE,
                    array( 'sprintf' => array( $appVersion->app_name ) ) ), 302 );
        }
        ...
        \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( 'versions_track', $form, FALSE );
    }
}